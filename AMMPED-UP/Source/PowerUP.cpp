...
void PowerUP::init(const sf::Texture& texture, sf::Vector2f pos, PowerUPType type)
{
	...
	{
		m_powerUPSprite.setTextureRect({ 9 * 48,0,48,48 });
		m_powerUPSprite.setScale(16.0f / 48.0f, 16.0f / 48.0f);
	...
	{
		m_powerUPSprite.setTextureRect({ 4 * 48,0,48,48 });
		m_powerUPSprite.setScale(16.0f / 48.0f, 16.0f / 48.0f);
	...
	{
		m_powerUPSprite.setTextureRect({ 12 * 48,0,48,48 });
		m_powerUPSprite.setScale(16.0f / 48.0f, 16.0f / 48.0f);

	}
	if (m_type == ICE_CREAM) //INCREASE SCORE BY 25
	{
		m_powerUPSprite.setTextureRect({ 0 * 48,0,48,48 });
		m_powerUPSprite.setScale(16.0f / 48.0f, 16.0f / 48.0f);
